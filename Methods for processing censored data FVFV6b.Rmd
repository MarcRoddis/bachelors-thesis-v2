---
title: "Methods for processing censored data FVFV6"
author: "Marc Roddis"
date: "7/23/2020"
output: 
  pdf_document:
...
# Determination of appropriate sample size and selection of censoring methods for further study
...
Moreover, these generally high values are commensurate with our prior expectations (ref: Helsel's book) that $\texttt{omit}$ is a poor method, so we will not study this method further.
...
This all makes sense since we are using a constant $LOD$ value for the whole 15-year period for every dataset.
...
This explains why substitution by the highest value $LOD$ gives predictions with increasing variance as year increases, whereas the opposite is true for substitution by the lowest value $\frac {LOD}{2}$ by the same reasoning.

However, the following set of four graphs show that the variance from substitution by $\frac {LOD}{\sqrt{2}}$ decreases relative to that from $\texttt{censReg1}$ as $|\beta_{A}|$ increases.
...
This suggests that the substitution value $\frac{LOD}{2}$ is than the true values on average for $|\beta_{A}| = 0.02$ but not lower for the highest value $|\beta_{A}| = 0.16$.  

This conclusion is also supported by the fact that the bias from $\texttt{subst2}$ is much lower than that from $\texttt{subst1}$ or $\texttt{subst4}$, which suggests that the real values of the censored data mostly lie between $LOD$ and $\frac{LOD}{2}$ for these sets of parameter values.
