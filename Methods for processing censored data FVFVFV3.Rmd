---
title: "Methods for processing censored data FVFVFV3"
author: "Marc Roddis"
date: "8/3/2020"
output: 
  pdf_document:
...
The mathematical formulation for $\texttt{censReg2}$ corresponds to that presented above for $\texttt{censReg1}$, except that we model each observation $y_{i}^{*}$ as from a normal distribution with mean  $\mu_{i_{X,A}} = \alpha_{X, A} + \beta_{X}x_{i} + \beta_{A}a_{i}$ and variance $\sigma^{2}$.

This means that the likelihood function for the $\texttt{censReg2}$ model will have the same form as that for the $\texttt{censReg1}$ model; it will differ only in having $\mu_{i_{X,A}}$ in place of $\mu_{i_{X}}$.
...
This means that the censored $y_{i}$ are imputed by $\hat{y}_{i}$, where

$$\hat{y}_i = - \hat{\sigma} \frac{ exp[((-1/2)( \hat {\alpha}_{X} + \hat {\beta}_{X}x_{i} + \hat {\beta}_{A}a_{i} - \text{LOQ}) / \hat{\sigma})^{2}]}{[1- ( \Phi(  \hat {\alpha}_{X} + \hat {\beta}_{X}x_{i} + \hat {\beta}_{A}a_{i} - \text{LOQ}) / \hat{\sigma}  ) ]\sqrt{2\pi} } + \hat {\alpha}_{X} + \hat {\beta}_{X}x_{i} + \hat {\beta}_{A}a_{i}$$
